...
def mallado(num_mall, limx, limy):
    ...
    plt.title('Mallado de densidades', fontsize=22, pad=20)
...
def rayeq(t, f, l, L, w, nc):
    ...
    nden, gxn, gyn, gzn = ndens(xr, yr, zr, L, radioblanco, center)
...
# Gráfica del mapa de densidad electrónica
def show(L):
    ...
    for i in range(NX):
        for j in range(NY):
            nd[j, i], gxn, gyn, gzn = ndens(X[i], Y[j], 0, L, radioblanco, center)
    ...
    # Ajusta la resolución (dpi) para mejorar la calidad al hacer zoom en la imagen descargada
    plt.savefig('Mapa de densidad.png', dpi=2000)

radiolente = 10  # Radio del lente
NR = 50  # número de rayos (número de rectas)
Rss = 18.e-3  # Radio de la sección de estudio
...
mallado(numerodeparticiones, limitesx, limitesy)
show(L)
